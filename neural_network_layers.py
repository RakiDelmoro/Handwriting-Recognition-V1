...
def convolution_neurons(input_image, axons, dentrites, linear_axons, step_of_patch_window):
    ...
    flatten_feature_extracted = image_featured_extracted.reshape(batch, -1)
    output_layer = linear_neurons_activations(flatten_feature_extracted, linear_axons)
    return output_layer
...
def attention_mechanism_neurons(input_neurons, num_attn_heads, attn_head_feature_size, layer_1_connections, layer_2_connections, layer_3_connections):
    ...
    linear_1_projection = apply_attn_heads_dim(linear_neurons_activations(input_neurons, axons=layer_1_connections[0], dentrites=layer_1_connections[1]))
    linear_2_projection = apply_attn_heads_dim(linear_neurons_activations(input_neurons, axons=layer_2_connections[0], dentrites=layer_2_connections[1]))
    linear_3_projection = apply_attn_heads_dim(linear_neurons_activations(input_neurons, axons=layer_3_connections[0], dentrites=layer_3_connections[1]))
    raw_attention_scores = (cupy.matmul(linear_1_projection, linear_2_projection.transpose(0, 1, 3, 2))) / math.sqrt(num_attention_heads*attn_head_feature_size)
