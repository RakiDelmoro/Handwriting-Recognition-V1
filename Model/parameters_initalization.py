...
def transformer_parameters_initializer(network_feature_size: int, mlp_architecture: list, output_class: list):
    ...
    def image_embeddings_parameters():
        ...
        classification_token_parameters = np.zeros((1, num_patches+2, network_feature_size), dtype=np.float32)
        distillation_token_parameters = np.zeros((1, num_patches+2, network_feature_size), np.float32)
        position_embeddings_parameters = np.zeros((1, num_patches+2, network_feature_size), np.float32)
        parameters = np.stack(embeddings_parameters, axis=0)
        learnable_tokens = np.stack([classification_token_parameters, distillation_token_parameters, position_embeddings_parameters], axis=0)
    ...
    def attention_layer_parameters():
        axons = np.zeros(shape=(3, network_feature_size, network_feature_size))
        dentrites = np.zeros(shape=(3, network_feature_size, network_feature_size))
        for each in range(3):
        ...
        transformer_parameters['attention_parameters'] = np.stack([axons, dentrites], axis=0)

    def encoder_mlp_layer_parameters():
        axons = np.zeros(shape=(3, network_feature_size*MLP_RATIO, network_feature_size*MLP_RATIO))
        dentrites = np.zeros(shape=(3, network_feature_size*MLP_RATIO, network_feature_size*MLP_RATIO))
        ...
        transformer_parameters['encoder_mlp_parameters'] = np.stack([axons, dentrites], axis=0)
    ...
    def mlp_layer_parameters():
        ...
        for layer_idx in range(len(mlp_architecture)-1):
            ...
            axons[layer_idx, :input_size, :output_size] = layer_axons
            dentrites[layer_idx, :input_size, :output_size] = layer_dentrites
        transformer_parameters['mlp_parameters'] = np.stack([axons, dentrites], axis=0)

    def output_layer_parameters():
       parameters = linear_initialization(network_feature_size, output_class)
       transformer_parameters['output_parameters'] = np.stack(parameters, axis=0)
